...
from ..consumers.employee_consumer import EmployeesConsumer
...
from models.messaging_models.readdb_model import BaseReadDbModel
...
class ConsumersHandler:
        ...
        try:
            ...
            async with msg.process():
                async with AsyncInfraDbLocalSession() as session:
                    ...
                    if res:
                        ic(res.get("set_response"),res.get("response"),res.get("mark_completed"))
                        if res.get("set_response"):
                            await SagaStatesRepo(session=session).merge(service="accounts",data=res.get("response"),saga_id=saga_id)
                        ...
                        if res.get('read_db') and readdb_cls:
                            ...
                            handler=getattr(readdb_cls,read_db_infos.method)
                            if not handler:
            ...
            async with AsyncInfraDbLocalSession() as session:
                ...
                await SagaStatesRepo(session=session).update_step(key=f"{service_name}.{action}",status=SagaStepsValueEnum.FAILED,saga_id=saga_id)
    ...
    async def main_handler(msg:AbstractIncomingMessage):
        ...
        ic(f"Name => {domain}\nWork-For => {work_for}\nAction => {action}\nstate => {state}\nVersion => {version}")
        if f"{domain}.{work_for}"=='employees.employees':
            await ConsumersHandler._helper(consumer_cls=EmployeesConsumer,msg=msg,service_name=SERVICE_NAME.lower(),exchange_service_name=domain)
        else:
            ic("Unknown event")